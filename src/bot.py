...
async def cmd_myguardian(m: types.Message, delete_previous: types.Message = None) -> None:
    uid = m.from_user.id
    logger.info(f"🔍 /guardian вызвана с user_id={uid}")
    
    if delete_previous:
        try:
            await bot.delete_message(delete_previous.chat.id, delete_previous.message_id)
        except:
            pass
...
async def cmd_stats(m: types.Message, delete_previous: types.Message = None):
    if delete_previous:
        try:
            await bot.delete_message(delete_previous.chat.id, delete_previous.message_id)
        except:
...
async def cmd_check(m: types.Message) -> None:
    ...
    if risks:
        ...
        prompt = (
            f"Контракт {addr} на opBNB. GoPlus выявил следующие риски: {', '.join(risks)}. "
            f"Дай оценку безопасности. Ответь строго в формате JSON:\n"
            f"- verdict: одно из [\"SAFE\", \"WARNING\", \"DANGER\"]\n"
            f"- confidence: число от 0 до 1\n"
            f"- risk_factors: массив строк (можно использовать переданные риски)\n"
            f"- explanation: краткое пояснение на русском\n"
            f"Только JSON, без дополнительного текста."
        )
    else:
        icon, status = "✅", "Явных угроз не обнаружено"
        prompt = (
            f"Дан контракт {addr} на opBNB. GoPlus не обнаружил явных угроз. "
            f"Твоя задача: дать краткую оценку безопасности этого контракта. "
            f"Ответь строго в формате JSON со следующими полями:\n"
            f"- verdict: одно из [\"SAFE\", \"WARNING\", \"DANGER\"] (SAFE = безопасно, WARNING = есть сомнения, DANGER = опасно)\n"
            f"- confidence: число от 0 до 1, насколько ты уверен в вердикте\n"
            f"- risk_factors: массив строк с потенциальными рисками (если нет, оставь пустым)\n"
            f"- explanation: краткое пояснение для пользователя на русском (2-3 предложения)\n"
            f"Только JSON, без дополнительного текста."
        )
...
async def cmd_status(m: types.Message, delete_previous: types.Message = None) -> None:
    if delete_previous:
        try:
            await bot.delete_message(delete_previous.chat.id, delete_previous.message_id)
        except:
...
async def cmd_limit(m: types.Message, delete_previous: types.Message = None) -> None:
    if m.text is None:
        if delete_previous:
            try:
                await bot.delete_message(delete_previous.chat.id, delete_previous.message_id)
...
async def handle_ask_ai(m: types.Message) -> None:
    clear_state(m.from_user.id)
    wait = await bot.send_message(m.chat.id, "⏳ AI думает...")
    async with ai_sem:
